...
def load_json(app_id, context_id, steam_id: str, api_key=None, overwrite=False) -> dict:
    ...
    if os.path.exists(inventory_file_path) and not overwrite:
        ...
        if not inventory_validator.validate_format(inventory):
            raise SystemExit(
                f"Invalid inventory data in {inventory_file_path}. Run with --overwrite to download again."
            )
        if not inventory_validator.validate_size(inventory):
            raise SystemExit(
                f"Empty inventory in {inventory_file_path}. Run with --overwrite to download again."
            )
    else:
